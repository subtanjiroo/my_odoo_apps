...
export class ChatController extends FormController {    
    ...
    async sendMessage() {
        ...
        try {
            ...
            const response = await fetch(`/leandix_ai/get_direct_answer?${params.toString()}`, {
                method: 'GET',
                cache: 'no-store',
            ...
            this.value.forEach((conversation) => {
                ...
                nameDiv.addEventListener("click",async () => {
                    ...
                    if (this.showDeletePanel == false) {
                        ...
                        let vvv = await this.orm.call("leandix.ai.session", "save_current_chat_id", [this.current_convesation_id]);
                        this.loadMess();
                        this.hide_something();
