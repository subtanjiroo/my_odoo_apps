...
class chat_model(models.Model):
    _name = 'leandix.ai.base.chat.model'
    ...
    def send_message_to_engine_api(self, message, history, chat_model, user_id,error = None):
        try:
            ...
            payload = {
                ...
                "error": error or 'None',
                "lang": user_lang,
                "current_chat_id": f"{current_convs_id}",
            }
            ...
            if status_code == 200:
                ...
                return get_error_message(int(status_code),user_lang)

        except urllib.error.HTTPError as e:
            _logger.info(f"[get_error_message]: {e}")
            _logger.info(f"Payload sent to FastAPI: {json.dumps(payload)}")

            return get_error_message(int(e.code),user_lang)
    ...
    def send_message_to_answer_api_stream_prepared(self, payload, api_url):
        try:
            data_bytes = json.dumps(payload).encode("utf-8")
    ...
    def naming_and_create_conversation(self, prompt, chat_model, userid):
        try:
            ...
            _logger.exception("HTTPError when creating conversation")
            return e.code
        except Exception as e:
            _logger.exception("Unexpected error when creating conversation")
            return 500
                ...
                if current_user_id is not None and tables:
                    ...
                    if sql_command == "SELECT":
                        ...
                        for i, table in enumerate(tables):
                            ...
                            has_user_domain = any("('user_id','=',user.id)" in domain for domain in domain_list)
                            logging.info(f"has_full_access: {has_user_domain}")
